...
#'
#' Graphically display model selection criteria and component and/or transition probabilities of the optimal fitted model of \code{\link{class}} \code{search_lqmix}
#'
...
plot.search_lqmix = function(x, ...){
    ...
    if(x$method == "bic"){
      ...
      par(mar=c(5, 4, 4, 2) + 0.1,mfrow=c(1,1), xpd = TRUE)
      matplot(xx, cbind(bbicv), type="b", pch = x$Gv, lty=x$Gv, add = F,col = x$Gv, xaxt="n", xlab="Number of states", ylab = "", main = "Model selection criterion: BIC")
      legend("topright",horiz = F, legend=c(paste("G=", x$Gv, sep="")),lty=x$Gv,bty="n",
             col = x$Gv, pch = c(x$Gv), cex = 0.8)

    }else if(x$method == "aic"){
      aaicv = matrix(x$aicv, length(x$mv), length(x$Gv), byrow = T)
      xx = matrix(rep(1:length(x$mv), each = length(x$Gv)), ncol = length(x$Gv), byrow = T)
      par(mar=c(5, 4, 4, 2) + 0.1,mfrow=c(1,1), xpd = TRUE)
      matplot(xx, cbind(aaicv), type="b", pch = x$Gv, lty=x$Gv, add = F,col = x$Gv, xaxt="n", xlab="Number of states", ylab = "", main = "Model selection criterion: AIC")
      ...
      xx = matrix(rep(1:length(x$mv), each = length(x$Gv)), ncol = length(x$Gv), byrow = T)
      par(mar=c(5, 4, 4, 2) + 0.1,mfrow=c(1,1), xpd = TRUE)
      matplot(xx, cbind(llkv), type="b", pch = x$Gv, lty=x$Gv,add = F, col = x$Gv, xaxt="n", xlab="Number of states", ylab = "", main = "Model selection criterion: logLik")
  ...
  if(x$optimal$model == "TC"){
    ...
    devAskNewPage(ask = T)
      par(mar = c(0,2,2,2))
      pg = round(diag(x$optimal$pg),2)
      plotmat(pg,relsize=0.7,
              name = as.character(1:x$optimal$G), box.col="lightgray",
